...
class ProductProduct(models.Model):
    ...
    image_main_id = fields.Many2one(comodel_name='ir.attachment', string="Main image", compute='_get_v_image_main_id', store=True)
    image_main_medium_id = fields.Many2one(comodel_name='ir.attachment', string="Medium image", compute='_get_v_image_main_id', store=True)
    image_main_small_id = fields.Many2one(comodel_name='ir.attachment', string="Small image", compute='_get_v_image_main_id', store=True)
    image_attachment_ids = fields.Many2many(string='Images', comodel_name='ir.attachment')
    ...
    def _get_v_image_main_id(self):
        ...
        if self.image_attachment_ids:
            ...
            self.image_main_id = self.product_tmpl_id.image_attachment_ids and self.product_tmpl_id.image_attachment_ids.sorted(lambda r: r.sequence)[0]
        image = self.image_main_id.with_context(bin_size=False).datas
        try:
            images = tools.image_get_resized_images(image)
            images = {
